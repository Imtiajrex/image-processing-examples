...
def apply_gaussian_lowpass_filter(image, sigma=None):
    ...
    for i in range(height):
        for j in range(width):
            ...
            mask[i, j] = math.exp(-(distance**2) / (2 * sigma**2))
...
image_path = '20701014_Q10/20701014_Q10_input.jpg'
preprocessed_image = preprocess_image(image_path)
if preprocessed_image is not None:
    filtered_image = apply_gaussian_lowpass_filter(preprocessed_image, sigma=None)
    filtered_image_pil = Image.fromarray(filtered_image)
    filtered_image_pil.save('20701014_Q10/20701014_Q10_output.jpg')